...
plugin.collectionType(Collection.prototype);
plugin.provider(Collection.prototype);
plugin.errors(Collection.prototype, 'Collection');
...
/**
 * Set collection types for the collection.
 *
 * @param {String} `plural` e.g. `products`
 * @param {Object} `options`
 */

Collection.prototype.collectionType = function(types) {
  this.options.collectionType = utils.arrayify(this.options.collectionType);
  ...
  while (len--) {
    ...
    if (['collection'].indexOf(type) === -1) {
      throw new Error('"' + type + '" is not a valid collectionType. Valid collectionTypes are: collection.');
    }

    if (this.options.collectionType.indexOf(type) === -1) {
      this.options.collectionType.push(type);
    }
  }

  if (this.options.collectionType.length === 0) {
    this.options.collectionType.push('collection');
  }
  return this.options.collectionType;
};
